<?php
	...
	*/
	...
	if( !class_exists('infinite_theme_landing') ){
		class infinite_theme_landing{
			...
			function __construct( $settings = array() ){
				...
				if( class_exists('gdlr_core_admin_option') ){
					...
					gdlr_core_admin_menu::register_menu(array(
						'main-menu' => true,
						'main-menu-title' => esc_html__('Goodlayers', 'infinite'),
						'parent-slug' => '',
						'page-title' => $this->settings['page-title'], 
						'menu-title' =>$this->settings['menu-title'], 
						'capability' => $this->settings['capability'], 
				...
				add_action('infinite_tgmpa_no_item_redirect', array(&$this, 'tgmpa_no_item_redirect'));
			}
				...
				if( !empty($_GET['page']) && $_GET['page'] == $this->settings['slug'] ){
					if( !empty($_POST['purchase_code']) ){
						try{
							...
							infinite_verify_purchase($_POST['purchase_code'], $register);
							wp_redirect(admin_url('admin.php?page=' . $this->settings['slug']));
							exit;
						}catch(Exception $e){
							$this->error_message = $e->getMessage();
							if( empty($this->error_message) ){
								$this->error_message = esc_html__('Unknown Error, please contact adminiatrator', 'infinite');
							}
						}
					}
				}
			...
			function enqueue_script($hook){
				if( strpos($hook, 'page_' . $this->settings['slug']) !== false ){
					...
					wp_enqueue_style('gdlr-core-font-awesome', get_template_directory_uri() . '/plugins/font-awesome/css/font-awesome.min.css');
					wp_enqueue_style('gdlr-core-getting-start', get_template_directory_uri() . '/admin/installer/css/getting-start.css');
					wp_enqueue_style('infinite-open-sans-css', 'https://fonts.googleapis.com/css?family=Open+Sans:400,300,300italic,400italic,600,600italic,700,700italic,800,800italic&subset=latin,latin-ext');
					...
					wp_localize_script('gdlr-core-getting-start', 'gdlr_core_ajax_message', array(
						...
						'error_head' => esc_html__('An error occurs', 'infinite'),
						'error_message' => esc_html__('Please try again. If the problem still persists, please contact administrator for this.', 'infinite'),
						'nonce' => wp_create_nonce('gdlr_core_demo_import'),

						'importing_head' => esc_html__('Importing demo content. Please wait...', 'infinite'),
						'importing_content' => esc_html__('If you choose to download images from demo site, it can take up to 7-8 minutes so please be patient.', 'infinite'),
					));	

					// init variable
					$this->is_verified = infinite_is_purchase_verified();
				}				
			}	
			...
			function get_theme_landing_content(){
				...
				$content['registration'] = array(
					'title' => esc_html__('Registration', 'infinite'),
					'type' => 'registration'
				);
				...
				$content['support'] = array(
					...
					'content' => wp_kses(__('<p><strong>Have questions about how to use the theme?</strong></p><p>Make sure that you read all through the document we provided. The document is contained in main package that you downloaded from Themeforest. It\'s in the folder \'Document-Instruction\', open the file \'index.html\' with your browser. Or you can view online document here.</p>', 'infinite'), array('strong'=>array(), 'br'=>array(), 'p'=> array()) ) . 
						'<a class="gdlr-core-button" target="_blank" href="http://demo.goodlayers.com/document/infinite" >' . esc_html__('View Online Document', 'infinite') . '</a><br><br>' .
			...
			function get_header(){
				...
				$this->is_verified = infinite_is_purchase_verified();
				if( $this->is_verified ){
					echo '<div class="gdlr-core-getting-start-header-content infinite-verified" >';
					echo '<p>';
					echo '<i class="fa fa-check" ></i>' . esc_html__('License Registered!', 'infinite');
					echo '<span>(<a href="https://goodlayers.com/licenses/" target="_blank">' . esc_html__('manage your license', 'infinite') . '<i class="fa fa-external-link"></i></a>)</span>';
					echo '</p>';
					
					echo '<h3 class="gdlr-core-getting-start-header-title" >' . esc_html__('Congratulations !', 'infinite') . '</h3>';
					echo '<div class="gdlr-core-getting-start-header-caption" >';
					echo '<strong>' . esc_html__('infinite', 'infinite') . '</strong> ' . esc_html__('WordPress theme is now installed and ready to use. Thank you so much for choosing Goodlayers!', 'infinite');
					echo '</div>';
				}else{
					echo '<div class="gdlr-core-getting-start-header-content infinite-not-verified" >';
					echo '<p><i class="fa fa-close" ></i>' . esc_html__('Unregistered', 'infinite') . '</p>';
					echo '<h3 class="gdlr-core-getting-start-header-title" >' . esc_html__('Please Register Your Website', 'infinite') . '</h3>';
					echo '<div class="gdlr-core-getting-start-header-caption" >';
					echo esc_html__('Infinite requires license registration in order to install the theme\'s core features, import demo content and to be able to update the theme’s core functionalities in the future.', 'infinite');
					echo '</div>';
				}
			...
			function get_registration(){
				...
				if( $this->is_verified ){
					echo '<div class="infinite-registered" >';
					echo '<h3><i class="fa fa-lock" ></i>' . esc_html__('Your website is registered', 'infinite') . '</h3>';
					echo '<p>';
					echo '<strong>' . esc_html__('You can manage your site\'s registration(such as revoking) via our license portal here :', 'infinite') . '</strong>';
					echo '<a href="https://goodlayers.com/licenses/" target="_blank">https://goodlayers.com/licenses/</a> ';
					echo '<span>(' . esc_html__('You need to create a new account. The account is not the same account in our support portal.', 'infinite') .')<span>';
					echo '</p>';

					$purchase_code = infinite_get_purchase_code();
					echo '<strong class="infinite-large" >' . esc_html__('Your Purchase Code', 'infinite') . ':</strong>';
					echo '<form id="infinite-registration" method="post" >';
					echo '<div class="infinite-input-placeholder" >';
					echo gdlr_core_text_filter($purchase_code);
					echo '</div>';
					echo '<input type="hidden" name="purchase_code" value="' . esc_attr($purchase_code) . '" />';
					echo '<input type="submit" value= "' . esc_html__('Refresh Status', 'infinite') . '" />';
					echo '</form>';
					echo '</div>';
					...
					if( !empty($this->error_message) ){
						echo '<div class="infinite-notification" ><i class="fa fa-info-circle"></i><p>';
						echo gdlr_core_text_filter($this->error_message);
						echo '</p></div>';
					}else if( function_exists('init_goodlayers_core_system') ){
						echo '<div class="infinite-notification" ><i class="fa fa-info-circle"></i><p>';
						echo esc_html__('You will still be able to use the theme without registration but you will NOT be able to update the core features and the slider. To ensure that you website works smoothly with latest WordPress version and to get latest security improvements, please register your website.', 'infinite');
						echo '</p></div>';
					}

					echo '<h3><i class="fa fa-lock" ></i>' . esc_html__('Register Your Website', 'infinite') . '</h3>';
					echo '<p class="infinite-large" >' . esc_html__('Please enter your theme\'s purchase code to install the theme\'s core features, import demo content and to be able to update the theme\'s core functionalities in the future.', 'infinite') . '</p>';
					
					echo '<form id="infinite-registration" method="post" >';
					echo '<input type="text" name="purchase_code" placeholder="' . esc_html__('Fill purchase code. Ex. baf6XXXX-b2BB-XXXX-cAAb-22XXXXf357A', 'infinite') . '" />';
					echo '<input type="hidden" name="register" value="1" />';
					echo '<input type="submit" value="' . esc_html__('Register Now', 'infinite') . '" />';
					echo '</form>';

					echo '<h4><i class="fa fa-question-circle"></i>' . esc_html__('How to get my purchase code?', 'infinite') . '</h4>';
					echo '<ul>';
			...
			function get_demo_import($content){
				...
				echo '<span class="gdlr-core-head">' . esc_html__('Select Demo', 'infinite') . '</span>';
				echo '</div>';
				...
				echo '<a href="' . esc_url($first_url) . '" class="gdlr-core-view-demo-button" id="gdlr-core-view-demo-button" target="_blank">' . esc_html__('View Demo', 'infinite') . '<i class="fa fa-external-link" ></i></a>';
				...
				echo '<span class="gdlr-core-head">' . esc_html__('Import Settings', 'infinite') . '</span>';
				echo '</div>';
				...
				$image_license_content .= '<br><br>';
				$image_license_content .= wp_kses(__('* We will not assume any responsibility if you fail to follow our instructions, and legal actions are taken against your website by the copyright owner of the photos or their representatives.' , 'infinite'), array('strong' => array()));
				
				echo '<div class="gdlr-core-demo-import-option" >';
				echo '<input type="checkbox" data-name="image" checked >';
				echo '<span class="gdlr-core-option-text" >' . esc_html__('Download images from demo site', 'infinite') . ' ( <a href="#" id="gdlr-core-image-condition" >' . esc_html__('read conditions', 'infinite') . '</a> )</span>';
				...
				echo '<div class="gdlr-core-condition-close" ><i class="fa fa-remove" ></i>' . esc_html__('Close', 'infinite') . '</div>';
				echo '</div>';
				echo '</div>';

				$js_image_license_content  = wp_kses(__('You\'re downloading images from our demo website. Please read the statement below carefully.', 'infinite'), array('strong' => array()));
				$js_image_license_content .= '<br><br>';
				$js_image_license_content .= $image_license_content;
				$js_image_license_content .= '<br><br>';
				$js_image_license_content .= wp_kses(__('Please confirm if you want to proceed this.' , 'infinite'), array('strong' => array()));
				$js_image_license_content .= '<br><br>';
				$js_image_license_content .= wp_kses(__('* Please noted that this process could not be undone.' , 'infinite'), array('strong' => array()));
				echo '<script>';
				...
				echo '<span class="gdlr-core-option-text" >' . esc_html__('Import theme options', 'infinite') . ' ( <span class="gdlr-core-red">' . esc_html__('Noted that the current theme option will be overridden', 'infinite') . '</span> )</span>';
				echo '</div>';

				echo '<div class="gdlr-core-demo-import-option" >';
				echo '<input type="checkbox" data-name="widget" checked >';
				echo '<span class="gdlr-core-option-text" >' . esc_html__('Import widget (sidebar & footer)', 'infinite') . '</span>';
				echo '</div>';

				echo '<a class="gdlr-core-demo-import-button" id="gdlr-core-demo-import-submit" >' . esc_html__('Start Import Demo!', 'infinite') . '</a>';
				echo '</div>';
				echo '</div>'; // gdlr-core-demo-import-section-wrap
			...
			function get_system_status(){
				...
				echo '<div class="gdlr-core-system-status-head" >' . esc_html__('System Status', 'infinite') . '</div>';
				echo '<table><tbody>';

				// debug
				echo '<tr>';
				echo '<td class="gdlr-core-table-head" >' . esc_html__('Debug Mode', 'infinite') . '</td>';
				echo '<td class="gdlr-core-table-content" >';
				if( defined('WP_DEBUG') && WP_DEBUG ){
					echo esc_html__('On', 'infinite');
					echo '<span class="gdlr-core-recommendation">' . esc_html__('You should turn debug mode off when you make your site live', 'infinite') . '</div>';
				}else{
					echo esc_html__('Off', 'infinite');
				}
				echo '</td>';
				echo '</tr>';
				...
				// php version
				echo '<tr>';
				echo '<td class="gdlr-core-table-head" >' . esc_html__('PHP Version', 'infinite') . '</td>';
				echo '<td class="gdlr-core-table-content" >';
				...
				// wp upload max size
				echo '<tr>';
				echo '<td class="gdlr-core-table-head" >' . esc_html__('wp_max_upload_size', 'infinite') . '</td>';
				echo '<td class="gdlr-core-table-content" >';
				...
				if( function_exists('ini_get') ){
					...
					// upload max size
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('upload_max_filesize', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					...
					// post max size
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('post_max_size', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					...
					// max execution time
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('max_execution_time', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					$max_execution_time = ini_get('max_execution_time');
					echo gdlr_core_escape_content($max_execution_time);
					if( $max_execution_time < 300 ){
						echo '<span class="gdlr-core-recommendation">' . esc_html__('Recommend to be over 300 for demo import process', 'infinite') . '</div>';
					}
					echo '</td>';
					echo '</tr>';
					...
					// memory limit
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('memory_limit', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					$memory_limit = ini_get('memory_limit');
					echo gdlr_core_escape_content($memory_limit);
					$memory_limit = intval(str_replace('M', '', $memory_limit));
					if( $memory_limit < 512 ){
						echo '<span class="gdlr-core-recommendation">' . esc_html__('Recommend to be 512M for demo import process', 'infinite') . '</div>';
					}
					echo '</td>';
					echo '</tr>';
					...
					// max input var
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('max_input_vars', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					$max_input_vars = ini_get('max_input_vars');
					echo gdlr_core_escape_content($max_input_vars);
					if( $max_input_vars < 4000 ){
						echo '<span class="gdlr-core-recommendation">' . esc_html__('Recommend value is 4000 for full demo import process', 'infinite') . '</div>';
					}
					echo '</td>';
					echo '</tr>';
					...
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('default_socket_timeout', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					$default_socket_timeout = ini_get('default_socket_timeout');
					echo gdlr_core_escape_content($default_socket_timeout);
					// if( $default_socket_timeout < 300 ){
					// 	echo '<span class="gdlr-core-recommendation">' . esc_html__('Recommend value to be over 300 for images import process', 'infinite') . '</div>';
					// }
					echo '</td>';
					echo '</tr>';
					...
					// suhosin
					echo '<tr>';
					echo '<td class="gdlr-core-table-head" >' . esc_html__('suhosin', 'infinite') . '</td>';
					echo '<td class="gdlr-core-table-content" >';
					...
					if( extension_loaded( 'suhosin' ) ){
						...
						// post max var
						echo '<tr>';
						echo '<td class="gdlr-core-table-head" >' . esc_html__('suhosin.post.max_vars', 'infinite') . '</td>';
						echo '<td class="gdlr-core-table-content" >';
						...
						// request max var
						echo '<tr>';
						echo '<td class="gdlr-core-table-head" >' . esc_html__('suhosin.request.max_vars', 'infinite') . '</td>';
						echo '<td class="gdlr-core-table-content" >';
				...
				echo '<div class="gdlr-core-system-status-footer" >' . esc_html__('You can change these values by editing php.ini file directly. Or you can ask your hosting provider to do it for you.', 'infinite') . '</div>';
				echo '</div>';
			}

		} // infinite_theme_landing
	} // class_exists
